<div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 hover:shadow-md transition overflow-hidden">
    ...
    <a href="{{ route('orders.show', $transaction->order_id) }}" class="block p-3 hover:bg-gray-50 dark:hover:bg-gray-700/30 transition">
        <div class="flex justify-between items-start mb-2">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ $transaction->transaction_date->format('d/m H:i') }}</span>
        </div>
        <p class="text-sm font-medium text-gray-900 dark:text-white mb-1">{{ $transaction->category }}</p>
        <p class="text-xs text-gray-600 dark:text-gray-400 mb-2 line-clamp-2">{{ $transaction->description }}</p>
        ...
        <div class="mb-2 space-y-1">
            ...
            <div class="flex justify-between text-xs">
                <span class="text-gray-500 dark:text-gray-400 capitalize">{{ str_replace('_', ' ', $method['method'] ?? $transaction->payment_method) }}</span>
                <span class="text-gray-600 dark:text-gray-400">R$ {{ number_format($method['amount'] ?? $transaction->amount, 2, ',', '.') }}</span>
            </div>
            @endforeach
        </div>
        @endif
        ...
        <div class="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700">
            <span class="text-sm font-bold {{ $transaction->type === 'entrada' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Vendedor: {{ $transaction->user_name ?? 'Sistema' }}
        </div>
    </a>
    <div class="px-3 pb-3">
        <a href="{{ route('cash.edit', $transaction) }}" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline" onclick="event.stopPropagation()">Editar</a>
    </div>
    @else
    <div class="p-3">
        <div class="flex justify-between items-start mb-2">
        ...
        <div class="flex justify-between items-center pt-2 border-t border-gray-100 dark:border-gray-700">
            ...
            <a href="{{ route('cash.edit', $transaction) }}" class="text-xs text-indigo-600 dark:text-indigo-400 hover:underline">Editar</a>
        </div>
        <div class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Vendedor: {{ $transaction->user_name ?? 'Sistema' }}
        </div>

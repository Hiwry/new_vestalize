<div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
    @foreach($products as $product)
        <div onclick="openModal('{{ $product->id }}', '{{ $product->name }}', '{{ $product->price }}', {{ $product->addons->isNotEmpty() ? json_encode($product->addons) : '[]' }})" 
            ...
            <div class="mb-3 aspect-square rounded-xl bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden relative">
                @if($product->image)
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-2 capitalize">{{ $product->category }}</p>
            
            <div class="flex justify-between items-center">
                <span class="font-extrabold text-pink-600 dark:text-pink-400">R$ {{ number_format($product->price, 2, ',', '.') }}</span>

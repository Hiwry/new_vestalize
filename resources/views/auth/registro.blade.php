    ...
    <div class="max-w-4xl w-full grid md:grid-cols-2 bg-slate-900 rounded-3xl overflow-hidden shadow-2xl border border-slate-800">
        ...
        <div class="p-8 md:p-12">
            ...
            <form action="{{ route('register.public.post') }}" method="POST" class="space-y-5">
                ...
                <div>
                    ...
                    <select name="plan_id" required 
                        ...
                        @foreach($plans as $plan)
                            <option value="{{ $plan->id }}">Plano {{ $plan->name }} (R$ {{ number_format($plan->price, 2, ',', '.') }}/mês)</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex items-start gap-3 bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                    ...
                    <label for="terms" class="text-xs text-slate-400 leading-relaxed">
                        Eu li e aceito os <a href="#" class="text-indigo-400 hover:underline">Termos e Condições de Uso</a> e a <a href="#" class="text-indigo-400 hover:underline">Política de Privacidade</a>.
                    </label>
                </div>

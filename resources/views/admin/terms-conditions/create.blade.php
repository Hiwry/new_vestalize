...
<div class="flex justify-between items-center mb-6">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Criar Termos e Condições</h1>
        ...
        <form action="{{ route('admin.terms-conditions.store') }}" method="POST" class="bg-white dark:bg-gray-800 shadow rounded-lg border border-gray-200 dark:border-gray-700 p-6">
            @csrf
                ...
                <div>
                    ...
                    <select id="personalization_type" 
                        ...
                        @foreach($personalizationTypes as $key => $label)
                            <option value="{{ $key }}" {{ old('personalization_type') == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="fabric_type_id" 
                        ...
                        @foreach($fabricTypes as $fabric)
                            <option value="{{ $fabric->id }}" {{ old('fabric_type_id') == $fabric->id ? 'selected' : '' }}>
                                {{ $fabric->name }}
                            </option>
                        @endforeach
                    </select>

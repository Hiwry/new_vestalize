...
<div class="mb-8">
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Modelos</h1>
            <p class="text-gray-600 dark:text-gray-400 mt-2">Gerencie os modelos do catálogo</p>
        </div>
        <a href="{{ route('admin.modelos.create') }}" 
           class="px-6 py-3 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors flex items-center shadow-lg hover:shadow-xl">
            ...
            <span class="font-medium">Novo Modelo</span>
        </a>
    </div>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        ...
        <a href="{{ route('admin.personalizacoes.index') }}" 
           class="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors flex items-center shadow-md hover:shadow-lg">
            ...
            <span class="font-medium">Personalizações</span>
        </a>
    </div>
</div>
            ...
            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $modelo->name }}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-500 dark:text-gray-400">{{ Str::limit($modelo->description ?? '-', 50) }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-900 dark:text-gray-100">{{ $modelo->order }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($modelo->active)
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <a href="{{ route('admin.modelos.edit', $modelo) }}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-4">Editar</a>
                        <form action="{{ route('admin.modelos.destroy', $modelo) }}" method="POST" class="inline" onsubmit="return confirm('Tem certeza que deseja excluir este modelo?');">
                            @csrf
                            @method('DELETE')

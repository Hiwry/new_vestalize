...
@section('content')
<div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100">Editar Personalização</h1>
    <p class="text-gray-600 dark:text-gray-400 mt-2">Edite as informações da personalização</p>
</div>

<div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/25 p-6">
    <form action="{{ route('admin.personalizacoes.update', $personalizacao) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" 
                       id="name" 
                       name="name" 
                       value="{{ old('name', $personalizacao->name) }}"
                       required
            ...
            <div>
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3"
                          class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100">{{ old('description', $personalizacao->description) }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" 
                           id="order" 
                           name="order" 
                           value="{{ old('order', $personalizacao->order) }}"
                           min="0"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-700 dark:text-gray-100">
                </div>
                ...
                <div class="flex items-center pt-8">
                    <input type="checkbox" 
                           ...
                           name="active" 
                           value="1"
                           {{ old('active', $personalizacao->active) ? 'checked' : '' }}

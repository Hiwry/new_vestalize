...
<div class="max-w-5xl mx-auto">
    ...
    <form action="{{ route('budget.convert-to-order', $budget->id) }}" method="POST" enctype="multipart/form-data" id="convert-form">
        ...
        @php
            $itemMeta = is_string($item->personalization_types) ? (json_decode($item->personalization_types, true) ?? []) : ($item->personalization_types ?? []);
            $itemExistingSizes = is_array($itemMeta['sizes'] ?? null) ? $itemMeta['sizes'] : [];
        @endphp
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 mb-6 overflow-hidden">
                    ...
                    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-3">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" 
                                   ...
                                   min="0" 
                                   value="{{ (int) ($itemExistingSizes[$size] ?? $itemExistingSizes[strtolower($size)] ?? 0) }}"

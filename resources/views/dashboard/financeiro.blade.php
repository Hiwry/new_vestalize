...
<div class="mb-6 space-y-4">
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.invoices.index') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-medium text-sm text-white hover:bg-indigo-700 transition shadow-sm">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <!-- Filtro de Período -->
        <form method="GET" action="{{ route('financial.dashboard') }}" id="periodFilterForm" class="flex flex-wrap items-center gap-2">
        ...
        <form method="GET" action="{{ route('financial.dashboard') }}" id="storeFilterForm" class="flex flex-wrap items-center gap-2">
            @if(isset($period))
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/25 p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">R$ {{ number_format($totalRevenue, 2, ',', '.') }}</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/25 p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">R$ {{ number_format($totalCost, 2, ',', '.') }}</p>
            </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow dark:shadow-gray-900/25 p-6">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-3xl font-bold {{ $profitMargin >= 0 ? 'text-blue-600 dark:text-blue-400' : 'text-red-600 dark:text-red-400' }}">
            ...
            <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                <div class="flex-1 min-w-0 mr-4">
                    <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                        {{ $product->print_type ?? 'Produto s/ nome' }}
                    </p>
                ...
                <div class="text-right">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">
                        Margem: {{ $product->revenue > 0 ? number_format(($product->profit / $product->revenue) * 100, 1) : 0 }}%
                    </p>
                </div>
            </div>
            @empty
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($itemsReport as $item)
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                            {{ $item->print_type ?? 'N/A' }}
                        </td>
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-indigo-600 dark:text-indigo-400">
                            <a href="{{ route('orders.show', $item->order_code) }}" class="hover:underline">
                                #{{ str_pad($item->order_code, 6, '0', STR_PAD_LEFT) }}
                            </a>
                            <div class="text-xs text-gray-500">{{ \Carbon\Carbon::parse($item->order_date)->format('d/m/Y') }}</div>
                        </td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-right font-bold {{ ($item->total_price - $item->total_cost) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            R$ {{ number_format($item->total_price - $item->total_cost, 2, ',', '.') }}
                        </td>
                    </tr>
                    @empty
                    <tr>

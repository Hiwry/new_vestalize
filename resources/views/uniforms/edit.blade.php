    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-2xl font-semibold text-gray-900 dark:text-gray-100">Editar Item</h1>
            </div>
            
            <form action="{{ route('uniforms.destroy', $uniform) }}" method="POST" onsubmit="return confirm('ATENÇÃO: Tem certeza que deseja excluir este item permanentemente?');">
                @csrf
                @method('DELETE')
        ...
        {{-- Formulário --}}
        <form action="{{ route('uniforms.update', $uniform) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <select name="store_id" required class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                            @foreach ($stores as $store)
                                <option value="{{ $store->id }}" {{ old('store_id', $uniform->store_id) == $store->id ? 'selected' : '' }}>{{ $store->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="type" value="{{ old('type', $uniform->type) }}" list="types_list" required
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                    ...
                    <div>
                        ...
                        <input type="text" name="color" value="{{ old('color', $uniform->color) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="size" value="{{ old('size', $uniform->size) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                    </div>
                    <div>
                        ...
                        <select name="gender" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                            ...
                            <option value="Masculino" {{ old('gender', $uniform->gender) == 'Masculino' ? 'selected' : '' }}>Masculino</option>
                            <option value="Feminino" {{ old('gender', $uniform->gender) == 'Feminino' ? 'selected' : '' }}>Feminino</option>
                        </select>
                    </div>
                     <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Quantidade *</label>
                        <input type="number" step="0.001" name="quantity" value="{{ old('quantity', $uniform->quantity) }}" required
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        ...
                        <input type="number" step="0.001" name="min_stock" value="{{ old('min_stock', $uniform->min_stock) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Observações</label>
                        <textarea name="notes" rows="1"
                                  class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg">{{ old('notes', $uniform->notes) }}</textarea>
                    </div>
                </div>
            </div>

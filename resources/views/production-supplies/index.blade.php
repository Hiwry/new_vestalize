...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-4 mb-6">
        <form action="{{ route('production-supplies.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div class="md:col-span-3">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Buscar</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Nome, cor, tipo..."
            ...
            <div class="flex gap-2">
                ...
                <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium self-end transition">Filtrar</button>
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Tipo/Cor</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse($products as $product)
                        @php
                            $key = $product->name . '|' . $product->type . '|' . $product->color . '|' . $product->unit;
                            $rowTotal = 0;
                        @endphp
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100 sticky left-0 bg-white dark:bg-gray-800 group-hover:bg-gray-50 dark:group-hover:bg-gray-700 shadow-sm z-10">
                                {{ $product->name }}
                                <div class="text-xs font-normal text-gray-500">{{ $product->unit }}</div>
                            </td>
                            
                            {{-- Detalhes --}}
                            <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                <div>{{ ucfirst($product->type) }}</div>
                                @if($product->color)
                                    <div class="text-xs">{{ $product->color }}</div>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ count($stores) + 5 }}" class="px-4 py-12 text-center text-gray-500 dark:text-gray-400">

...
<div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-xl dark:shadow-2xl dark:shadow-black/20 border border-gray-200 dark:border-slate-800">
            ...
            <!-- Resumo do Pedido -->
                <div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border border-indigo-200 dark:border-indigo-800 p-5 mb-6">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium text-gray-900 dark:text-white"><?php echo e($order->client->name); ?></span>
                        </div>
                        
                        <div class="flex justify-between">
                            <span class="text-gray-700 dark:text-slate-300">Total de Peças:</span>
                            <span class="font-medium text-gray-900 dark:text-white" id="total-pieces"><?php echo e($order->items->sum('quantity')); ?></span>
                        </div>
                        ...
                        <div class="border-t border-indigo-200 dark:border-indigo-800 pt-2 mt-2">
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white" id="subtotal">R$ <?php echo e(number_format($order->subtotal, 2, ',', '.')); ?></span>
                            </div>
                        </div>
                        ...
                        <div class="border-t border-indigo-200 dark:border-indigo-800 pt-2 mt-2">
                            <div class="flex justify-between text-base font-bold">
                                ...
                                <span id="total-final" class="text-indigo-600 dark:text-indigo-400">R$ <?php echo e(number_format($order->subtotal, 2, ',', '.')); ?></span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <!-- Formulário -->
                <form method="POST" action="<?php echo e(request()->routeIs('orders.edit.*') ? route('orders.edit.payment') : route('orders.wizard.payment')); ?>" id="payment-form" class="space-y-6" enctype="multipart/form-data">
                    <?php echo csrf_field(); ?>
                    ...
                    <input type="hidden" name="order_data" value="<?php echo e(json_encode($order->items->first()->sizes ?? [])); ?>">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="bg-gray-50 dark:bg-slate-800/50 rounded-md p-4">
                            <input type="date" id="entry_date" name="entry_date" required
                                   value="<?php echo e($sessionPaymentData['entry_date'] ?? $order->payments->first()->entry_date ?? $order->entry_date ?? date('Y-m-d')); ?>"
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="bg-gray-50 dark:bg-slate-800/50 rounded-md p-4">
                            ...
                            <input type="number" id="delivery_fee" name="delivery_fee" step="0.01" min="0" value="<?php echo e($sessionPaymentData['delivery_fee'] ?? $order->delivery_fee ?? 0); ?>"
                    ...
                    <div class="flex justify-between pt-6 border-t border-gray-200 dark:border-slate-800">
                        <a href="<?php echo e(request()->routeIs('orders.edit.*') ? route('orders.edit.customization') : route('orders.wizard.customization')); ?>" 
...
<script>
        let paymentMethods = [];
        let subtotal = <?php echo e($order->subtotal); ?>;
        let deliveryFee = <?php echo e($sessionPaymentData['delivery_fee'] ?? $order->delivery_fee ?? 0); ?>;
        let sizeSurcharges = {};
        let orderItems = <?php echo json_encode($order->items, 15, 512) ?>;
        let discountType = 'none';
        let discountValue = 0;

        document.addEventListener('DOMContentLoaded', function() {
            // Priorizar dados da sessão (se o usuário voltou de uma etapa posterior)
            <?php if(isset($sessionPaymentData) && !empty($sessionPaymentData['payment_methods'])): ?>
                <?php
                    $sessionMethods = json_decode($sessionPaymentData['payment_methods'], true);
                ?>
                <?php if($sessionMethods && is_array($sessionMethods)): ?>
                    <?php $__currentLoopData = $sessionMethods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        paymentMethods.push({
                            id: <?php echo e($pm['id'] ?? 'Date.now() + Math.random()'); ?>,
                            method: '<?php echo e($pm["method"] ?? "pix"); ?>',
                            amount: <?php echo e($pm["amount"] ?? 0); ?>

                        });
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    renderPaymentMethods();
                <?php endif; ?>
            <?php elseif($order->payments && $order->payments->count() > 0): ?>
                <?php
                ...
                ?>
                <?php if($existingMethods && is_array($existingMethods)): ?>
                    <?php $__currentLoopData = $existingMethods; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pm): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        paymentMethods.push({
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\vestalize.10 (1)\vestalize.10\resources\views/orders/wizard/payment.blade.php ENDPATH**/ ?>